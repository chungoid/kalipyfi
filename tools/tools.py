...
class Tool:
    def __init__(self, name: str, description: str, base_dir: Path,
        ...
        self.sync_connected_wlan_interfaces()
    ...
    ######################################
### CONFIG ###
    def sync_connected_wlan_interfaces(self) -> None:
        """
        Checks the 'interfaces' section under "wlan" key in the configuration file.
        If no wlan interfaces defined in the config are currently connected,
        updates the configuration by auto-adding connected wlan interfaces.
    ...
    def sync_connected_ethernet_interfaces(self) -> None:
        """
        Checks the 'interfaces' section under the "eth" key in the configuration file.
        If no ethernet interfaces defined in the config are currently available,
        updates the configuration by auto-adding available ethernet interfaces.
        Uses get_available_ethernet_interfaces and update_yaml_value helpers.
        """
        from tools.helpers.tool_utils import get_available_ethernet_interfaces, update_yaml_value
        import yaml
        ...
        # Ensure the 'interfaces' section and 'eth' category exist as a list.
        if "interfaces" not in config:
            config["interfaces"] = {}
        if "eth" not in config["interfaces"] or not isinstance(config["interfaces"]["eth"], list):
            config["interfaces"]["eth"] = []

        # Get available ethernet interfaces.
        available_eth = get_available_ethernet_interfaces(self.logger)
        self.logger.debug(f"Available ethernet interfaces: {available_eth}")
        if not available_eth:
            self.logger.info("No ethernet interfaces are currently available.")
            return

        # Build a set of existing interface names from config.
        existing_names = {entry.get("name") for entry in config["interfaces"]["eth"] if entry.get("name")}
        self.logger.debug(f"Existing ethernet interface names in config: {existing_names}")

        # Prepare new entries for each available ethernet interface not already in config.
        new_entries = []
        for iface in available_eth:
            if iface not in existing_names:
                new_entry = {
                ...
                config["interfaces"]["eth"].append(new_entry)
                new_entries.append(iface)
                self.logger.info(f"Auto-adding ethernet interface '{iface}' to configuration.")

        # If new entries were added, update the configuration file.
        if new_entries:
            try:
                update_yaml_value(self.config_file, ["interfaces", "eth"], config["interfaces"]["eth"])
                self.logger.info(f"Auto-added ethernet interfaces: {', '.join(new_entries)}")
            except Exception as e:
                self.logger.error(f"Error updating configuration file {self.config_file}: {e}")

        # Reload in-memory configuration.
        self.reload_config()
