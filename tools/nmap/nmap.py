...
class Nmap(Tool, ABC):
    ...
    ##### UTILITIES #####
    #####################
    def run_searchsploit(self) -> None:
        """
        Runs searchsploit using the XML file from the scan.
        The output is written to searchsploit_results.txt in the current working directory.
        """
        xml_file = self._determine_xml_file_path()
        if not xml_file:
        ...
        cmd = ["searchsploit", "--nmap", str(xml_file)]
    ...
    def _determine_xml_file_path(self) -> Optional[Path]:
        """
        Searches the current working directory for an .xml file.
        Assumes that the nmap command generated an XML file using -oA.
        """
        self.logger.debug(f"Checking for .xml files in: {self.current_working_dir}")
        xml_files = list(Path(self.current_working_dir).glob("*.xml"))
        if xml_files:
            self.logger.debug(f".xml files found: {[str(f) for f in xml_files]}")
            return xml_files[0]
        else:
            self.logger.error("No .xml files found in %s", self.current_working_dir)
            return None
